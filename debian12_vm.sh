...
: '
if whiptail --backtitle "Proxmox VE Helper Scripts" --title "Debian12VM" --yesno "This will create a New Debian12 VM. Proceed?" 10 58; then
  :
else
...
function default_settings() {
  ...
  echo -e "${CREATING}${BOLD}${DGN}Creating a Debian12 VM using the above default settings${CL}"
...
function advanced_settings() {
  ...
  if VM_NAME=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Set Hostname" 8 58 debian12 --title "HOSTNAME" --cancel-button Exit-Script 3>&1 1>&2 2>&3); then
    if [ -z $VM_NAME ]; then
      HN="debian12"
      echo -e "${HOSTNAME}${BOLD}${DGN}Hostname: ${BGN}$HN${CL}"
    else
  ...
  if (whiptail --backtitle "Proxmox VE Helper Scripts" --title "ADVANCED SETTINGS COMPLETE" --yesno "Ready to create a Debian12 VM?" --no-button Do-Over 10 58); then
    echo -e "${CREATING}${BOLD}${DGN}Creating a Debian12 VM using the above advanced settings${CL}"
  else
    header_info
...
msg_info "Retrieving the URL for the Debian12 Qcow2 Disk Image"
...
msg_info "Configuring the VM with virt-customize"
...
msg_info "Creating a Debian12 VM"
...
<div align='center'>
  ...
  <h2 style='font-size: 24px; margin: 20px 0;'>Debian12 VM</h2>
...
msg_ok "Created a Debian12 VM ${CL}${BL}(${HN})"
if [ "$START_VM" == "yes" ]; then
  msg_info "Starting Debian12 VM"
  qm start $VMID
  msg_ok "Started Debian12 VM"
fi
#post_update_to_api "done" "none"
