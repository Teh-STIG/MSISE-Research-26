...
OPTIONS:
    ...
    -b                  WAN Bridge (Default: vmbr0)
    -z                  LAN Bridge (Default: vmbr1)
...
while getopts ":h:v:d:n:c:or:b:zl:s:i:k:" opt; do
  case "${opt}" in
    h) show_help exit 0 ;;
    v) VMID=$OPTARG ;;
    d) DISK_SIZE=$OPTARG ;;
...
function default_settings() {
  ...
  #-o (CPU TYPE, default KVM64, else... uhh... KVM64?)
  if [ -z "$CPU_TYPE" ]; then
    CPU_TYPE=""  
	else
		CPU_TYPE="" 
  ...
  #-b (Default WAN bridge)
  if [ -z "$WAN_BRG" ]; then
    WAN_BRG="vmbr0"
  fi
  #-z (Default LAN bridge)
  if [ -z "$LAN_BRG" ]; then
    LAN_BRG="vmbr1"
  fi
#  IP_ADDR=""
...
#  WAN_NETMASK=""
  ...
  if ! ip link show "${LAN_BRG}" &>/dev/null; then
    msg_error "LAN Bridge '${LAN_BRG}' does not exist"
    exit
  else
    echo -e "${DGN}Using WAN Bridge: ${BGN}${LAN_BRG}${CL}"
  fi 
  echo -e "${MACADDRESS}${BOLD}${DGN}WAN MAC Address: ${BGN}${MAC}${CL}"
  echo -e "${MACADDRESS}${BOLD}${DGN}LAN MAC Address: ${BGN}${LAN_MAC}${CL}"
  ...
  #I NEED TO TAKE A LOOK AT THIS AFTER RUNNING IT THE FIRST TIME

  # Determine available network modes based on bridge count
  local DEFAULT_WAN_BRG
  DEFAULT_WAN_BRG=$(echo "$AVAILABLE_BRIDGES" | grep -v "^${LAN_BRG}$" | head -n1 || true)
...
qm create $VMID -agent 1${MACHINE} -tablet 0 -localtime 1 -bios ovmf${CPU_TYPE} -cores $CORE_COUNT -memory $RAM_SIZE \
  -name $HN -tags community-script -onboot 1 -ostype l26 -scsihw virtio-scsi-pci
...
qm set $VMID \
  -net0 virtio,bridge=${LAN_BRG},macaddr=${LAN_MAC}${VLAN}${MTU} 2>/dev/null
msg_ok "Bridge interfaces have been successfully added."
if [ -n "$WAN_BRG" ]; then
  msg_info "Adding WAN interface"
  qm set $VMID \
    -net1 virtio,bridge=${WAN_BRG},macaddr=${MAC} &>/dev/null
...
msg_ok "Waiting 7 minutes for everything to download, extract and install... if your OPNsesne VM is not properly configured after this step, it must have taken longer than 7 minutes an you will likely need to remove the OPNsesne VM and start the deploy script again!"
sleep 420
send_line_to_vm "root"
...
msg_ok "Installing QEMU Agent"
send_line_to_vm "root"
send_line_to_vm "opnsense"
send_line_to_vm "8"
send_line_to_vm "pkg install qemu-guest-agent"
send_line_to_vm "y"
send_line_to_vm 'sysrc qemu_guest_agent_enable="YES"'
send_line_to_vm "reboot"

msg_ok "QEMU Agent should now be installed, if the next step does not provide an IP address or states that the QEMU agent is not installed, you may need to manually install it..."
